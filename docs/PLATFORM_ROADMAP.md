...
لا واجهة للمنصّة اليوم إطلاقاً (`web/index.html` للمحرّك لا للمنصّة). تشمل:
لوحة `basic`/`full` (`entitlements.dashboard_level()` جاهزة)، قواميس الترجمة
المفتاحية + RTL، صفحة `reset-password`، وواجهة القمع. **يحتاج قرارات
شكل/هوية بصرية من المالك قبل البدء.**
