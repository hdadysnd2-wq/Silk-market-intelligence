...
PR-1 شحن `draft→in_progress` (الإطلاق) وحده. الحالتان `completed`/`archived`
موجودتان في قيد `CHECK` بالمخطّط لكن **لا مسار يصل إليهما**، وعمود `state` غير
مُدرَج في `repository._WRITABLE` عمداً (آلةُ حالاتٍ لا تُكتَب بـCRUD عامّ) — فلا
سبيل إليهما إلا من هنا.

**لماذا هذا ليس مسكاً للدفاتر:** عامل الطابور **لا يفحص حالة الدراسة إطلاقاً**
(`email_queue.py`). فلو كان الأرشفة مجرّد تغيير عمود، لظلّت رسائل الدراسة
المؤرشَفة **تُرسَل فعلياً وتُخصَم من المحفظة** بعد أن سحبها العميل — أي أن الحالة
تكون **ادعاءً كاذباً** عن الواقع. لذلك:

- **`completed` يُرفَض ما دام هناك بريدٌ معلّق.** «مكتملة» وفي الطابور رسائل
  ستخرج بعد قليل = وصفٌ مخالف للواقع. ينتظر العميل النفاد أو يؤرشف.
...
Archiving actually cancels pending sends — otherwise the state would be a claim
the system contradicts. In-flight ('sending') rows are worker-owned, never touched.
"""
