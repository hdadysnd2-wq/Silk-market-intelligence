...
def run_storage_billing(conn: sqlite3.Connection) -> list[dict]:
    ...
    Charges storage per account from image byte totals; no images ⇒ no charge.
    """
    rows = conn.execute(
    ...
    for r in rows:
        ...
        wallet.post_entry(conn, account_id=r["account_id"], actor_user_id=None,
                          ...
                          description="monthly storage billing",
